...
class Shimmer extends StatefulWidget {
  ...
  /// The radius of the corners of the widgett in logical pixels.
  ...
  /// Defaults to a size of 128 logical pixels wide and 28 logical pixels high.
  final Size size;

  /// The initial offset of the shimmering effect.
  /// Expressed as a fraction of the widget's width.
  /// Defaults to 0.0,
  /// meaning the shimmering effect will starts at the leading edge of the widget.
...
class _ShimmerState extends State<Shimmer> with SingleTickerProviderStateMixin {
  /// Initialize shader.
  static final Future<ui.FragmentShader?> _shaderFuture = ui.FragmentProgram.fromAsset(
    'assets/shaders/shimmer.frag',
  ).then<ui.FragmentShader?>((value) => value.fragmentShader(), onError: (_, __) => null);

  /// Seed value notifier for shader mutaion.
  ...
  void _updateSeed(Duration elapsed) => _seed.value = elapsed.inMicroseconds * widget.speed / 8000;
...
class _ShimmerPainter extends CustomPainter {
  ...
  final ValueNotifier<double> seed;
  final Color color;
  final Color backgroundColor;
  final double cornerRadius;
  final double stripeWidth;
